...
We use caching for `uv` dependencies to speed up CI runs cached runs complete in under 2 minutes versus 5+ minutes uncaced.Tests run on Ubuntu Linux with Python 3.11. We dont test multiple OS/Python versions as our deployment target is fixed (linux containers).
